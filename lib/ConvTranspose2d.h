struct ConvTranspose2d {
    ...
    ConvTranspose2d(const char * bias_s, const char * weight_s) {
    ...
    template<class TI, class TO> void operator() (std::vector<std::vector<std::vector<TI> > > & in_vec, std::vector<std::vector<std::vector<TO> > > & out_vec) {
        ...
        int sy = stride_y;
        int sx = stride_x;
        for(int y=0, y2=0, dy0=0; y<out_h; ++y) {
            for(int x=0, x2=0, dx0=0; x<out_w; ++x) {
                std::vector<TO> & out_vec2 = out_vec[y][x];
                for(int c=0; c<out_c; ++c) {
                    TO f = bias[c];
                    std::vector<std::vector<std::vector<float> > > weight2 = weight[c];
                    int y3 = y2 - ph;
                    for(int dy=dy0; dy<kh; dy+=sy, ++y3) {
                        if(y3<0 || in_h<=y3) {
                            continue;
                        }
                        int x3 = x2 - pw;
                        for(int dx=dx0; dx<kw; dx+=sx, ++x3) {
                            if(x3<0 || in_w<=x3) {
                                continue;
                            }
