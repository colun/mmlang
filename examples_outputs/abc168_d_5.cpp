...
int main() {
    ...
    while(!(F).empty()) { int f = (F).front(); (F).pop();
        for(const tuple<int, int> & $fr : _slice(lower_bound(AB.begin(), AB.end(), tuple<int, int>(f, -2147483648)), upper_bound(AB.begin(), AB.end(), tuple<int, int>(f, 2147483647)))) { const int & f2 = get<0>($fr); const int & t = get<1>($fr);
            if(C[t] == -1) {
                C[t] = C[f] + 1;
                F.push(t);
            }
