//#define NDEBUG
#ifdef mmlang_source_3_lines

S, X = inputWord(), ["SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"]
if X[s@:] == S:
    print(7 - s)
...
}std::string inputWord(){return inputCWord();}void print_unit(bool val){printf("%s",val?"true":"false");}void print_unit(char val){printf("%c",val);}void print_unit(int val){printf("%d",val);}void print_unit(unsigned int val){printf("%u",val);}void print_unit(size_t
...
int main() {
    {
        ...
        const auto & $c2 = vector<string>({"SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"});
        tie(S, X) = tuple<string, vector<string> >($c1, $c2);
    }
    for(int s=0, $e=(int)X.size(); s<$e; ++s) if(X[s] == S) {
        print(7 - s);
    }
    return 0;
}
