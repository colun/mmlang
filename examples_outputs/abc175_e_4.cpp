...
_range_t(std::max(b,e)-1,b-1,-1);}_range_t rrange(int b,int e,int s){assert(s!=0);if(0<s){e=b+(std::max(0,e-b-1)/ s+1)*s;}else{e=b+(std::max(0,b-e-1)/-s+1)*s;}return _range_t(e-s,b-s,-s);}template<class It> class _slice_t{private:It b;It e;public:_slice_t(It
b,It e):b(b),e(e){}It begin(){return b;}It end(){return e;}};template<class It> _slice_t<It> _slice(It b,It e){return _slice_t<It>(b,e);}void print_unit(bool val){printf("%s",val?"true":"false");}void print_unit(char val){printf("%c",val);}void print_unit(int
...
int main() {
    ...
    for(const int & r : range(R)) {
        for(const int & c : range(C)) {
            v = 0;
            for(auto & $1 : _slice(lower_bound(rcv.begin(), rcv.end(), tuple<int, int, int>(r, c, -2147483648)), upper_bound(rcv.begin(), rcv.end(), tuple<int, int, int>(r, c, 2147483647)))) v = get<2>(($1));
            for(const int & i : range(4)) {
                if(1 <= i) {
                    D[r][c][i] = max(D[r][c][i], (long long)(v));
                }
                if(1 <= r) {
                    if(1 <= i) {
                        for(int $1=0, $e=(0<=4 ? 4 : (int)D[r - 1][c].size()+4); $1<$e; ++$1) D[r][c][i] = max(D[r][c][i], D[r - 1][c][$1] + v);
                    }
                    else {
                        for(int $1=0, $e=(0<=4 ? 4 : (int)D[r - 1][c].size()+4); $1<$e; ++$1) D[r][c][i] = max(D[r][c][i], D[r - 1][c][$1]);
                    }
                }
                if(1 <= c) {
                    for(int $1=0, $e=(0<=i ? i : (int)D[r][c - 1].size()+i); $1<$e; ++$1) D[r][c][i] = max(D[r][c][i], D[r][c - 1][$1] + v);
                    for(int $e0=i + 1, $1=0, $e=(0<=$e0 ? $e0 : (int)D[r][c - 1].size()+$e0); $1<$e; ++$1) D[r][c][i] = max(D[r][c][i], D[r][c - 1][$1]);
                }
            }
        }
    }
