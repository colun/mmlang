...
int64 solve$origin(int r, int c, int i) {
    long long ret;
    ret = (1 <= i ? V[r][c] : 0LL);
    if(1 <= r) {
        if(i == 0) {
            for(int $1=0, $e=4; $1<$e; ++$1) ret = max(ret, (long long)(solve(r - 1, c, $1)));
        }
        else {
            for(int $1=0, $e=4; $1<$e; ++$1) ret = max(ret, (long long)(solve(r - 1, c, $1) + V[r][c]));
        }
    }
    if(1 <= c) {
        for(int $1=0, $e=i; $1<$e; ++$1) ret = max(ret, (long long)(solve(r, c - 1, $1) + V[r][c]));
        ret = max(ret, (long long)(solve(r, c - 1, i)));
    }
    return ret;
}
vector<int64> solve$memo;
int64 solve(int r, int c, int i) {
    if(solve$memo.empty()) {
        solve$memo.resize(R * C * 4, (unsigned int64)-1 >> 1);
    }
    auto & ref = solve$memo[r*C*4 + c*4 + i];
    if(ref == (unsigned int64)-1 >> 1) {
        ref = solve$origin(r, c, i);
    }
    return ref;
}
