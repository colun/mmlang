...
for f in range(1, N):
    V[:0] = 0, 0
    V.clear()
    V.emplace_back(C[t@(:AB.where(f))[1]], t)
    V.sort()
    print(V[0][1] + 1)
...
int main() {
    ...
    while(!(F).empty()) { int f = (F).front(); (F).pop();
        for(auto & $1 : _slice(lower_bound(AB.begin(), AB.end(), tuple<int, int>(f, -2147483648)), upper_bound(AB.begin(), AB.end(), tuple<int, int>(f, 2147483647)))) {
            const auto & t = get<1>(($1));
            if(C[t] == -1) {
                C[t] = C[f] + 1;
                F.push(t);
            }
