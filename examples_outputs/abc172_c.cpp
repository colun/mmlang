...
}int inputInt(){return atoi(inputCWord());}long long inputInt64(){return atoll(inputCWord());}void print_unit(bool val){printf("%s",val?"true":"false");}void print_unit(char val){printf("%c",val);}void print_unit(int val){printf("%d",val);}void print_unit(unsigned
...
int main() {
    {
        ...
        const auto & $c3 = inputInt64();
        tie(N, M, K) = tuple<int, int, long long>($c1, $c2, $c3);
    }
    {
        A.resize(max((int)A.size(), (int)N));
        for(int $1=0, $e=(0<=N ? N : (int)A.size()+N); $1<$e; ++$1) A[$1] = inputInt64();
    }
    {
        B.resize(max((int)B.size(), (int)M));
        for(int $1=0, $e=(0<=M ? M : (int)B.size()+M); $1<$e; ++$1) B[$1] = inputInt64();
    }
    for(int $1=(0<=1 ? 1 : (int)A.size()+1), $e=(0<=N ? N : (int)A.size()+N); $1<$e; ++$1) A[$1] += A[$1 - 1];
    for(int $1=(0<=1 ? 1 : (int)B.size()+1), $e=(0<=M ? M : (int)B.size()+M); $1<$e; ++$1) B[$1] += B[$1 - 1];
    ret = (int)(upper_bound(B.begin(), B.end(), K) - B.begin());
    for(int $1=0, $e=(int)A.size(); $1<$e; ++$1) if(A[$1] <= K) {
        ret = max((int)(ret), (int)($1 + 1 + (int)(upper_bound(B.begin(), B.end(), K - A[$1]) - B.begin())));
    }
    print(ret);
    return 0;
}
