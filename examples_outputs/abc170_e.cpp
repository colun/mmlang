...
vector<int> A;
vector<int> B;
vector<tuple<int, int> > CD;
vector<set<tuple<int, int> > > S;
set<tuple<int, int> > R;
...
int main() {
    ...
    }
    {
        ...
        for(int $1=0, $e=(0<=N ? N : (int)A.size()+N); $1<$e; ++$1) {
            const auto & $c1 = inputInt();
    ...
    {
        CD.resize(max((int)CD.size(), (int)Q));
        for(int $1=0, $e=(0<=Q ? Q : (int)CD.size()+Q); $1<$e; ++$1) {
            const auto & $c1 = inputInt() - 1;
            const auto & $c2 = inputInt() - 1;
            CD[$1] = tuple<int, int>($c1, $c2);
        }
    }
    {
        S.resize(max((int)S.size(), (int)2e5 + 8));
        for(int $e0=2e5 + 8, $1=0, $e=(0<=$e0 ? $e0 : (int)S.size()+$e0); $1<$e; ++$1) S[$1] = set<tuple<int, int> >();
    }
    for(int $1=0, $e=(int)B.size(); $1<$e; ++$1) S[B[$1]].insert(tuple<int, int>(A[$1], $1));
    R = set<tuple<int, int> >();
    for(int s=0, $e=(int)S.size(); s<$e; ++s) if(!S[s].empty()) {
        R.insert(*S[s].rbegin());
    }
    for(const tuple<int, int> & $fr : CD) { const int & C = get<0>($fr); const int & D = get<1>($fr);
        if(!S[B[C]].empty()) {
            R.erase(R.find(*S[B[C]].rbegin()));
        }
        if(!S[D].empty()) {
            R.erase(R.find(*S[D].rbegin()));
        }
        S[B[C]].erase(S[B[C]].find(tuple<int, int>(A[C], C)));
        S[D].insert(tuple<int, int>(A[C], C));
        if(!S[B[C]].empty()) {
            R.insert(*S[B[C]].rbegin());
        }
        if(!S[D].empty()) {
            R.insert(*S[D].rbegin());
        }
