...
int main() {
    N = inputInt();
    {
        L.resize(max((int)L.size(), (int)N));
        for(int $1=0, $e=(0<=N ? N : (int)L.size()+N); $1<$e; ++$1) L[$1] = inputInt();
    }
    cnt = 0;
    for(int k=0, $e=(0<=N ? N : (int)L.size()+N); k<$e; ++k) for(int j=0, $e=(0<=k ? k : (int)L.size()+k); j<$e; ++j) for(int i=0, $e=(0<=j ? j : (int)L.size()+j); i<$e; ++i) {
        const auto & $c1 = L[j];
        const auto & $c2 = L[k];
        if(L[k] < L[j] + L[i] && L[j] < L[i] + L[k] && L[i] < L[j] + L[k] && (L[i] != $c1 && $c1 != $c2 && $c2 != L[i])) {
            ++cnt;
        }
    }
    print(cnt);
