...
A, B = inputInt64(), int64(inputFloat() * 100 + .5)
print(A * B / 100)
...
}long long inputInt64(){return atoll(inputCWord());}double inputFloat(){return atof(inputCWord());}void print_unit(bool val){printf("%s",val?"true":"false");}void print_unit(char val){printf("%c",val);}void print_unit(int val){printf("%d",val);}void print_unit(unsigned
...
int main() {
    {
        ...
        const auto & $c2 = (long long)(inputFloat() * 100 + .5);
        tie(A, B) = tuple<long long, long long>($c1, $c2);
    }
    print(A * B / 100);
    return 0;
}
