...
int main() {
    ...
    W = vector<string>({"AC", "WA", "TLE", "RE"});
    for(int $1=0, $e=(int)W.size(); $1<$e; ++$1) revW[W[$1]] = $1;
    {
        C.resize(max((int)C.size(), (int)W.size()));
        for(int $e0=W.size(), $1=0, $e=(0<=$e0 ? $e0 : (int)C.size()+$e0); $1<$e; ++$1) C[$1] = 0;
    }
    for(int $1=0, $e=(int)S.size(); $1<$e; ++$1) ++C[revW[S[$1]]];
    for(int $1=0, $e=(int)W.size(); $1<$e; ++$1) print(W[$1], "x", C[$1]);
    return 0;
}
