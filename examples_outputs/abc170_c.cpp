...
int main() {
    {
        ...
        tie(X, N) = tuple<int, int>($c1, $c2);
    }
    {
        p.resize(max((int)p.size(), (int)N));
        for(int $1=0, $e=(0<=N ? N : (int)p.size()+N); $1<$e; ++$1) p[$1] = inputInt();
    }
    {
        D.resize(max((int)D.size(), (int)110));
        for(int $1=0, $e=(0<=110 ? 110 : (int)D.size()+110); $1<$e; ++$1) D[$1] = 0;
    }
    for(int $1=0, $e=(0<=N ? N : (int)p.size()+N); $1<$e; ++$1) D[p[$1] + 5] = 1;
