...
_range_t(std::max(b,e)-1,b-1,-1);}_range_t rrange(int b,int e,int s){assert(s!=0);if(0<s){e=b+(std::max(0,e-b-1)/ s+1)*s;}else{e=b+(std::max(0,b-e-1)/-s+1)*s;}return _range_t(e-s,b-s,-s);}inline int mod_mul(int x,int y,int p){__asm__ __volatile__("imul %%edx; idiv %2"
:"+a"(y),"+d"(x):"r"(p));if(x<0){x+=p;}return x;}inline int mod(int x,int p){if(p<=x){do{x-=p;}while(p<=x);}else if(x<0){do{x+=p;}while(x<0);}return x;}void print_unit(bool val){printf("%s",val?"true":"false");}void print_unit(char val){printf("%c",val);
...
int main() {
    ...
    ret = 1;
    if(P.empty() && K % 2 == 1) {
        for(int $1=0, $e=(0<=K ? K : (int)M.size()+K); $1<$e; ++$1) ret = mod_mul(ret, M[$1], 1e9 + 7);
    }
    else if(N == K) {
        for(int $1=0, $e=(int)M.size(); $1<$e; ++$1) ret = mod_mul(ret, M[$1], 1e9 + 7);
        for(int $1=0, $e=(int)P.size(); $1<$e; ++$1) ret = mod_mul(ret, P[$1], 1e9 + 7);
    }
    else {
        if(K % 2 == 1) {
            ret = mod(({ auto ret = P.back(); P.pop_back(); ret; }), 1e9 + 7);
            --K;
        }
        while(K) {
            if(M.size() < 2) {
                ret = mod_mul(ret, ({ auto ret = P.back(); P.pop_back(); ret; }), 1e9 + 7);
                ret = mod_mul(ret, ({ auto ret = P.back(); P.pop_back(); ret; }), 1e9 + 7);
            }
            else if(P.size() < 2) {
                ret = mod_mul(ret, -({ auto ret = M.back(); M.pop_back(); ret; }), 1e9 + 7);
                ret = mod_mul(ret, -({ auto ret = M.back(); M.pop_back(); ret; }), 1e9 + 7);
            }
            else if((long long)(M[M.size() - 1]) * M[M.size() - 2] < (long long)(P[P.size() - 1]) * P[P.size() - 2]) {
                ret = mod_mul(ret, ({ auto ret = P.back(); P.pop_back(); ret; }), 1e9 + 7);
                ret = mod_mul(ret, ({ auto ret = P.back(); P.pop_back(); ret; }), 1e9 + 7);
            }
            else {
