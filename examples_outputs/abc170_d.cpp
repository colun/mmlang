...
int main() {
    ...
    {
        D.resize(max((int)D.size(), (int)1e6 + 8));
        for(int $e0=1e6 + 8, $1=0, $e=(0<=$e0 ? $e0 : (int)D.size()+$e0); $1<$e; ++$1) D[$1] = 0;
    }
    for(int $1=0, $e=(0<=N ? N : (int)A.size()+N); $1<$e; ++$1) ++D[A[$1]];
    sort(A.begin(), A.end());
    ret = 0;
    for(const int & a : A) {
        --D[a];
        for(const int & k : range(1, 1001)) {
            if(a % k == 0) {
                if(D[k]) {
                    goto $L2;
                }
