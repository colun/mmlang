...
}int inputInt(){return atoi(inputCWord());}__int128 inputInt128(){const char*p=inputCWord();bool sign=false;if(*p=='-'){sign=true;++p;}__int128 ret=0;while('0'<=*p&&*p<='9'){ret=ret*10+(*p-'0');++p;}return sign?-ret:ret;}void print_unit(bool val){printf("%s"
...
int main() {
    N = inputInt();
    {
        ...
        for(int $1=0, $e=(0<=N ? N : (int)A.size()+N); $1<$e; ++$1) A[$1] = inputInt128();
    }
    ZERO = 0;
    for(int $1=0, $e=(0<=N ? N : (int)A.size()+N); $1<$e; ++$1) ZERO += A[$1] == 0;
    if(ZERO) {
        print(0);
        return 0;
    }
    S = (__int128)(1);
    for(const __int128 & a : A) {
        S *= a;
        if((__int128)(1e18) < S) {
            print(-1);
            return 0;
