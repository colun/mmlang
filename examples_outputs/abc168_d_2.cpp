...
int main() {
    ...
    while(!(F).empty()) { int f = (F).front(); (F).pop();
        for(auto & $1 : range(lower_bound(AB.begin(), AB.end(), tuple<int, int>(f, -2147483648))-AB.begin(), upper_bound(AB.begin(), AB.end(), tuple<int, int>(f, 2147483647))-AB.begin())) {
            const auto & t = get<1>(AB[$1]);
            if(C[t] == -1) {
                C[t] = C[f] + 1;
                F.push(t);
            }
