...
int main() {
    {
        ...
        const auto & $c2 = inputInt64();
        tie(N, D) = tuple<int, long long>($c1, $c2);
    }
    {
        X.resize(max((int)X.size(), (int)N));
        Y.resize(max((int)Y.size(), (int)N));
        for(int $1=0, $e=(0<=N ? N : (int)X.size()+N); $1<$e; ++$1) {
            const auto & $c1 = inputInt64();
            const auto & $c2 = inputInt64();
            tie(X[$1], Y[$1]) = tuple<long long, long long>($c1, $c2);
        }
    }
    sum = 0;
    for(int $1=0, $e=(0<=N ? N : (int)X.size()+N); $1<$e; ++$1) sum += X[$1] * X[$1] + Y[$1] * Y[$1] <= D * D;
    print(sum);
    return 0;
}
