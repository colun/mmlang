...
int main() {
    N = inputInt();
    {
        A.resize(max((int)A.size(), (int)N + 1));
        for(int $e0=N + 1, $1=0, $e=(0<=$e0 ? $e0 : (int)A.size()+$e0); $1<$e; ++$1) A[$1] = 0;
    }
    for(const int & i : range(1, N + 1)) {
        for(int $e0=N + 1, $d=i, $1=(0<=i ? i : (int)A.size()+i), $e=(0<=$e0 ? $e0 : (int)A.size()+$e0); 0<=$d ? $1<$e : $e<$1; $1+=$d) ++A[$1];
    }
    sum = 0LL;
    for(int $1=0, $e=(int)A.size(); $1<$e; ++$1) sum += (long long)(A[$1]) * $1;
    print(sum);
    return 0;
}
