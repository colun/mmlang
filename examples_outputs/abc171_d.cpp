...
int main() {
    ...
    Q = inputInt();
    {
        BC.resize(max((int)BC.size(), (int)Q));
        for(int $1=0, $e=(0<=Q ? Q : (int)BC.size()+Q); $1<$e; ++$1) {
            const auto & $c1 = inputInt();
            const auto & $c2 = inputInt();
            BC[$1] = tuple<int, int>($c1, $c2);
        }
    }
    S = 0LL;
    for(int $1=0, $e=(0<=N ? N : (int)A.size()+N); $1<$e; ++$1) S += A[$1];
    cnt = vector<int>(1e5 + 8);
    for(int $1=0, $e=(0<=N ? N : (int)A.size()+N); $1<$e; ++$1) ++cnt[A[$1]];
    for(const tuple<int, int> & $fr : BC) { const int & B = get<0>($fr); const int & C = get<1>($fr);
        S += (C - B) * cnt[B];
        cnt[C] += cnt[B];
        cnt[B] = 0;
        print(S);
