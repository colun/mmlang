...
int main() {
    ...
    {
        const auto & $c1 = N % 10;
        const auto & $c2 = vector<int>({2, 4, 5, 7, 9});
        const auto & $c3 = N % 10;
        const auto & $c4 = vector<int>({0, 1, 6, 8});
        if(find($c2.begin(), $c2.end(), $c1)!=$c2.end()) {
            print("hon");
        }
        else if(find($c4.begin(), $c4.end(), $c3)!=$c4.end()) {
            print("pon");
        }
        else {
            print("bon");
